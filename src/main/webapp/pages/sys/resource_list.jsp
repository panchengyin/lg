...
<html>
	...
	<body class="pos-r">
		...
		<div class="page-container">
			<div  class="pos-a" style="width:240px;left:20;height:200px">
			...
			<div style="margin-left:250px">
					<!-- 条件栏 -->
					<div class="text-c">
						<input type="text"  id="txt_search" placeholder="请输入资源名称" style="width:40%" class="input-text"  onkeypress="if(event.keyCode==13) {fnRefresh();return false;}">
					...
					<!-- 列表 -->
					<div class="mt-20">
						<table id="table_resource" class="table table-border table-bordered table-bg table-hover table-sort" width="100%">
							<thead>
								<tr class="text-c">
									<th width="25"><input type="checkbox" name="" value=""></th>
									<th width="130">资源名称</th>
									<th width="130">上级资源</th>
									<th >权限字符串</th>
									<th width="60">排序</th>
									<th width="80">状态</th>
									<th width="80">操作</th>
	...
	<script type="text/javascript">
		...
		var oTable=null;//表格
		var isTreeNull=false;//判断是否有数据
		var selectedTreeNodeId="";//选中节点ID
		var selectedTreeNodeName="";//选中节点名称
		...
		function initJsTree(){
			$.ajax({
			     ...
			     success: function(data) {
			    	...
			    	$('#orgjsTree').jstree({'plugins':["wholerow"], 'core' : {  
						 'data' : data
					}}); 
			     }
			 });	
		...
		$('#orgjsTree').bind("activate_node.jstree", function (obj, e) {  
		  	selectedTreeNodeId=e.node.id;
		  	selectedTreeNodeName=e.node.text;
		  	fnRefresh();
		});  
	
		function queryList(){
			oTable=$("#table_resource").dataTable({
				...
				"bPaginage":true,  //进度条
				"sAjaxSource": '<%=path%>/sysResourceController/list.do', 
				"aoColumns":[  
					{ 
					...
					{ "mData": "resourceName","sClass":"text-c"}, 
		        	{ "mData": "parentName","sClass":"text-c"}, 
		        	{ "mData": "permissionStr"},
		        	{ "mData": "orderNum","sClass":"text-c"},
		        	{ "mData": "isEnable","sClass":"text-c"},
		        	{ 
					"mData": null, "bSortable": false, "render":function(data,type,full){  
		                var content = '<div class="text-c">';
		       ...
		       "fnServerData" : function(sSource, aoData, fnCallback) {
 					...
 					aoData.push({"name" : "id","value" : selectedTreeNodeId});
		...
		//添加资源
		function add(){
			if(isTreeNull  && selectedTreeNodeId==""){
				layer.msg("请选中右侧列表!");
				return;
			}		 	
			var url="resource_add.jsp?isTreeNull="+isTreeNull+"&nodeId="+selectedTreeNodeId+"&nodeName="+selectedTreeNodeName;
			popFull("添加资源",url);
		}
		//修改资源
		function edit(id){
			popFull("编辑资源","resource_edit.jsp?id="+id);
		}
		/*删除*/
		function delById(table,id){
			var url="<%=path%>/sysResourceController/deleteById.do";
			layer.confirm('确认删除当前及相关记录？',function(index){	
				layer.close(index);			
				$.ajax({
		            type: "POST",
		            url:url + '?id=' + id,
	   ...
	   function batchDel(){  
	  		...
	  		if(ids!=''){
	  			...
	  			layer.confirm('确认批量删除记录？',function(index){	
					...
					$.ajax({
			            type: "POST",
			            url:"<%=path%>/sysResourceController/deleteById.do?id="+ids,
			            success: function(data) {
			               if(data.success){
							  layer.msg(data.msg,{icon:1,time:1500});
							  fnRefresh();
